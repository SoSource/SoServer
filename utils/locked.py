...
def check_validation_consensus(block, do_mark_valid=True, broadcast_if_unknown=False, downstream_worker=True, handle_discrepancies=True, backcheck=False, get_missing_blocks=True):
    ...
    if prev_block and prev_block.hash != block.previous_hash or sigData_to_hash(block) != block.hash:
        prnt(f'block.hash:{prev_block.hash}, block.previous_hash:{block.previous_hash}, sigData_to_hash(block):{sigData_to_hash(block)}, block.hash:{block.hash}')
        carry_on = False
        logEvent(f'block.hash:{prev_block.hash}, block.previous_hash:{block.previous_hash}, sigData_to_hash(block):{sigData_to_hash(block)}, block.hash:{block.hash}, handle_discrepancies:{handle_discrepancies}, prev_block.index:{prev_block.index}, block.index:{block.index}')
